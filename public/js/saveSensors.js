...
class SensorHandle extends Autodesk.Viewing.Extension {
    ...
    async main() {
        ...
        var xVal = 0
        var maxDataLength = 50 // number of dataPoints visible at any point
        this.chart.render()
        ...
        self.startUpdateView = setInterval(async () => {
            ...
            for (const key in self._sensorReference) {
                ...
                var random = getRandom(self._sensorReference[key].randomMin, self._sensorReference[key].randomMax)
               ...
               for (let key in self.datapts)
                ...
                let color = howBad(random, self._sensorReference[key].min, self._sensorReference[key].max)
                ...
                if (color == red || color == yellow && typeof self.warnings[self._sensorReference[key].id] === 'undefined') {
    ...
    load() {
        console.log('SensorHandle has been loaded')
        return true
    }
